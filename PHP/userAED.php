<?php
	...
	if ($type == "add"){
		...
		if (mysql_num_rows($query) > 0){
			...
			$query = mysql_query("INSERT INTO users (userTypeID, username, userpass, fullname, address, phoneNum, mobileNum, email, gender) VALUES ($userTypeID, \"$user\", \"$pass\", \"$fullname\", \"$address\", \"$phoneNum\", \"$mobileNum\", \"$email\", \"$gender\" )",$conn);
		...
		//echo "UPDATE users SET userTypeID = $userTypeID , username = '$user' , userpass = '$pass' , fullname = '$fullname' , address = '$address' , phoneNum = '$phoneNum' , mobileNum = '$mobileNum' , email = '$email' , gender = '$gender' WHERE usersID = $usersID";
	}else if ($type == "delete"){
