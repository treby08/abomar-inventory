<?php
	...
	if ($type == "add"){
		...
		mysql_query("UPDATE salesInvoice SET sq_quoteNo = '$sq_quoteNo' , sq_custID = '$sq_custID', sq_branchID = '$sq_branchID' , 
		prepBy = '$preparedBy', apprBy = '$approvedBy', sq_vat = $vat, totalAmt = '$totalAmt' WHERE sqID = $sqID",$conn);
		
		mysql_query("UPDATE salesInvoice_details SET isRemove=1 WHERE sqd_sqID=$sqID",$conn);
		//$myQuery = "";
		for ($i=0; $i < count($arr_sqDetails); $i++){
			$arrDetails = array();
			$arrDetails = explode("|",$arr_sqDetails[$i]);
			if ($arrDetails[4]=="undefined"){
				$sql = "INSERT INTO salesInvoice_details (`sqd_sqID`, `sqd_prodID`, `quantity`, `price`, `totalPurchase`, `sqd_dateTrans`, `sqd_timeTrans`,isRemove) VALUES ($sqID, ".$arrDetails[0].", ".$arrDetails[1].", ".$arrDetails[2].", ".$arrDetails[3].", NOW(), NOW(),0)";
			}else{
				$sql =  "UPDATE salesInvoice_details SET quantity=".$arrDetails[1].", totalPurchase=".$arrDetails[2].", price=".$arrDetails[3].", isRemove=0 
		...
		$sql = "SELECT MAX(sqID)+1 as sqID FROM salesInvoice";
		$query = mysql_query($sql,$conn) or die(mysql_error().' $sql '. __LINE__);
