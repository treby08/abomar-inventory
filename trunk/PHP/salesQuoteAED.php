<?php
	...
	if ($type == "add"){
		...
		for ($i=0; $i < count($arr_sqDetails); $i++){
			...
			$sql = "INSERT INTO salesQuote_details (`sqd_sqID`, `sqd_prodID`, `quantity`, `totalPurchase`, `sqd_dateTrans`, `sqd_timeTrans`) VALUES ($sqID, ".$arrDetails[0].", ".$arrDetails[1].", ".$arrDetails[2].", NOW(), NOW())";
			mysql_query($sql,$conn) or die(mysql_error().' $sql '. __LINE__);
		}
		...
		for ($i=0; $i < count($arr_sqDetails); $i++){
			...
			if ($arrDetails[3]=="undefined"){
				$sql = "INSERT INTO salesQuote_details (`sqd_sqID`, `sqd_prodID`, `quantity`, `totalPurchase`, `sqd_dateTrans`, `sqd_timeTrans`,isRemove) VALUES ($sqID, ".$arrDetails[0].", ".$arrDetails[1].", ".$arrDetails[2].", NOW(), NOW(),0)";
			}else{
				$sql =  "UPDATE salesQuote_details SET quantity=".$arrDetails[1].", totalPurchase=".$arrDetails[2].", isRemove=0 
				WHERE sqdID=".$arrDetails[3];
